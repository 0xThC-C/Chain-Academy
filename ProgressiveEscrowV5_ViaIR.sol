...
// COMPILE WITH: solc --via-ir --optimize --optimize-runs 200 ProgressiveEscrowV5_ViaIR.sol
// OR in hardhat.config.js: settings: { viaIR: true, optimizer: { enabled: true, runs: 200 } }
...
contract ProgressiveEscrowV5 is Ownable, Pausable, ReentrancyGuard {
    ...
    function _validateSessionInputs(
        bytes32 sessionId,
        address mentor,
        address paymentToken,
        uint256 amount,
    ...
    function _initializeSession(
        ...
        s.releasedAmount = 0;
        s.sessionDuration = durationMinutes;
        s.startTime = 0;
        s.lastHeartbeat = 0;
        s.pausedTime = 0;
    ...
    function releaseProgressivePayment(bytes32 sessionId) external nonReentrant {
        ...
        // Ultra-simplified calculation to reduce stack depth
        ...
        require(maxRel > s.releasedAmount, "No payment");
