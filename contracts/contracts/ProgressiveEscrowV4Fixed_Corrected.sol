...
contract ProgressiveEscrowV4Fixed_Corrected is Ownable, Pausable, ReentrancyGuard {
    ...
    function createProgressiveSession(bytes32 sessionId, address mentor, address paymentToken, uint256 amount, uint256 durationMinutes, uint256 nonce) external payable nonReentrant whenNotPaused {
        ...
        s.sessionId = sessionId; s.student = msg.sender; s.mentor = mentor; s.paymentToken = paymentToken;
        s.totalAmount = amount; s.releasedAmount = 0; s.sessionDuration = durationMinutes; s.startTime = 0;
        s.lastHeartbeat = 0; s.pausedTime = 0; s.createdAt = block.timestamp; s.status = SessionStatus.Created;
        s.isActive = false; s.isPaused = false; s.surveyCompleted = false;
    ...
    function startProgressiveSession(bytes32 sessionId) external nonReentrant {
        ...
        s.status = SessionStatus.Active; s.startTime = block.timestamp; s.lastHeartbeat = block.timestamp;
        s.isActive = true; s.isPaused = false;
    ...
    function pauseSession(bytes32 sessionId) public nonReentrant {
        ...
        s.isPaused = true; s.status = SessionStatus.Paused;
    ...
    function resumeSession(bytes32 sessionId) public nonReentrant {
        ...
        s.pausedTime += block.timestamp - s.lastHeartbeat;
        s.isPaused = false; s.status = SessionStatus.Active; s.lastHeartbeat = block.timestamp;
        emit SessionResumed(sessionId, block.timestamp);
    }
    ...
    function releaseProgressivePayment(bytes32 sessionId) external nonReentrant {
        ...
        uint256 elapsed = _computeElapsed(sessionId);
