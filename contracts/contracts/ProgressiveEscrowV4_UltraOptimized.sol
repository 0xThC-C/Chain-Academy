...
contract ProgressiveEscrowV4_UltraOptimized is Ownable, Pausable, ReentrancyGuard {
    ...
    function createProgressiveSession(bytes32 sessionId, address mentor, address paymentToken, uint256 amount, uint256 durationMinutes, uint256 nonce) external payable nonReentrant whenNotPaused {
        require(mentor != address(0) && mentor != msg.sender && supportedTokens[paymentToken] && amount > 0 && durationMinutes > 0, "Invalid params");
        require(sessions[sessionId].student == address(0) && nonce == userNonces[msg.sender], "Session exists or nonce");

        userNonces[msg.sender]++;
        if (paymentToken == ETH_TOKEN) { require(msg.value == amount, "ETH mismatch"); } 
        else { require(msg.value == 0, "No ETH for ERC20"); IERC20(paymentToken).safeTransferFrom(msg.sender, address(this), amount); }
    ...
    function releaseProgressivePayment(bytes32 sessionId) external nonReentrant {
        ...
        // Inline transfer
        if (s.paymentToken == ETH_TOKEN) {
            (bool success, ) = payable(s.mentor).call{value: releaseAmount}("");
            require(success, "ETH failed");
        } else {
            IERC20(s.paymentToken).safeTransfer(s.mentor, releaseAmount);
        }
    ...
    function completeSession(bytes32 sessionId, uint256 rating, string calldata) external nonReentrant {
        ...
        if (mentorAmount > 0) {
            if (s.paymentToken == ETH_TOKEN) {
                (bool success, ) = payable(s.mentor).call{value: mentorAmount}("");
                require(success, "ETH failed");
            } else {
                IERC20(s.paymentToken).safeTransfer(s.mentor, mentorAmount);
            }
        }
        ...
        if (fee > 0) {
            if (s.paymentToken == ETH_TOKEN) {
                ...
                IERC20(s.paymentToken).safeTransfer(platformWallet, fee);
            }
        }
    ...
    function cancelSession(bytes32 sessionId) external nonReentrant {
        ...
        // Inline transfer
        if (s.paymentToken == ETH_TOKEN) {
            (bool success, ) = payable(s.student).call{value: refund}("");
            require(success, "ETH failed");
        } else {
            IERC20(s.paymentToken).safeTransfer(s.student, refund);
        }
    }
    ...
    function getAvailablePayment(bytes32 sessionId) external view returns (uint256) {
        ...
        if (elapsed == 0 || s.status != SessionStatus.Active || s.isActive == false || s.isPaused || s.sessionDuration == 0)
            return 0;
