...
/**
 * @title ProgressiveEscrowV4
 * @dev Revolutionary progressive payment system for mentorship sessions with native ETH support
 * 
 * Payment Flow:
 ...
 * - Supports both ERC20 tokens (USDT/USDC) and native ETH payments
 */
contract ProgressiveEscrowV4 is ReentrancyGuard, Pausable, Ownable {
    ...
    struct ProgressiveSession {
        ...
        address student;
        address mentor;
        address paymentToken;    // USDC, USDT, or address(0) for ETH
        uint256 totalAmount;
    ...
    /**
     * @dev Add supported payment token (USDC/USDT) or enable ETH
     */
    function addSupportedToken(address token) external onlyOwner {
        // token can be address(0) for ETH or a valid ERC20 address
    ...
    function createProgressiveSession(
        ...
        if (paymentToken == ETH_TOKEN) {
            ...
            require(paymentToken != address(0), "Invalid ERC20 token address");
     ...
     * ULTRATHINK: Gas-efficient lazy payment system
     */
    ...
    receive() external payable {
        ...
        // This is mainly for manual funding or emergency situations
    }
