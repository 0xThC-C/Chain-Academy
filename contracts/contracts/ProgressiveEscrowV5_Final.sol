...
contract ProgressiveEscrowV5 is Ownable, Pausable, ReentrancyGuard {
    ...
    function createProgressiveSession(
        ...
        _validateSessionInputs(sessionId, mentor, paymentToken, amount, durationMinutes, nonce);
        _handleSessionPayment(paymentToken, amount);
        _initializeSession(sessionId, mentor, paymentToken, amount, durationMinutes);
    }

    function _validateSessionInputs(bytes32 sessionId, address mentor, address paymentToken, uint256 amount, uint256 durationMinutes, uint256 nonce) private {
    ...
    function _initializeSession(bytes32 sessionId, address mentor, address paymentToken, uint256 amount, uint256 durationMinutes) private {
        usedSessionIds[sessionId] = true;
        ProgressiveSession storage s = sessions[sessionId];
        s.sessionId = sessionId; s.student = msg.sender; s.mentor = mentor; s.paymentToken = paymentToken; s.totalAmount = amount;
        s.releasedAmount = 0; s.sessionDuration = durationMinutes; s.startTime = 0; s.lastHeartbeat = 0; s.pausedTime = 0;
        s.createdAt = block.timestamp; s.status = SessionStatus.Created; s.isActive = false; s.isPaused = false; s.surveyCompleted = false;
    ...
    function releaseProgressivePayment(bytes32 sessionId) external nonReentrant {
        ...
        // Simplified calculation to reduce stack depth
        uint256 maxRel = (s.totalAmount * 90 * _computeElapsed(sessionId)) / (s.sessionDuration * 100);
        ...
        _doTransfer(s.paymentToken, s.mentor, releaseAmount);
    ...
    function updateHeartbeat(bytes32 sessionId) external nonReentrant {
        ...
        if (s.isPaused) {
            ...
            s.isPaused = false; s.status = SessionStatus.Active;
    ...
    function _finalizeSession(bytes32 sessionId) private {
        ProgressiveSession storage s = sessions[sessionId];
        s.status = SessionStatus.Completed; s.surveyCompleted = true; s.isActive = false;
        ...
        if (mentorAmount > 0) _doTransfer(s.paymentToken, s.mentor, mentorAmount);
        if (fee > 0) _doTransfer(s.paymentToken, platformWallet, fee);
    ...
    function _computeMaxRelease(uint256 total, uint256 elapsed, uint256 duration) internal pure returns (uint256) {
        ...
        return maxRel > (total * 90) / 100 ? (total * 90) / 100 : maxRel;
    }
    ...
    function _computeElapsed(bytes32 sessionId) internal view returns (uint256) {
        ...
        if (s.isPaused && s.lastHeartbeat > 0) pausedTime += block.timestamp - s.lastHeartbeat;
        return totalTime <= pausedTime ? 0 : (totalTime - pausedTime) / 60;
    }
    ...
    function getAvailablePayment(bytes32 sessionId) external view returns (uint256) {
        ...
        uint256 elapsed = (totalTime - pausedTime) / 60;
